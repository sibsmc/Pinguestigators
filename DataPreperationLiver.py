...
NNdataPan=NNdataPan.drop(columns=['Fat_Percentage_pancreas', 'Pathology_Categories_pancreas'])
short_data2=NNdataPan.copy()

from sklearn.preprocessing import MultiLabelBinarizer
mlb = MultiLabelBinarizer()
mask = short_data2['Pathology_Categories_liver'].notnull()
arr = mlb.fit_transform(short_data2.loc[mask, 'Pathology_Categories_liver'].dropna().str.split(', '))
...
Final_Data=Final_Data.drop(columns=['Age_Bracket','Hardy_Scale','Sex', 'Pathology_Categories_liver'])

# Final_Data.to_csv(r'insert_path', index=False)
